# -*- coding: utf-8 -*-
"""
Repository layer for LeaveRequest (thuần DB, ép alias erp_postgres):
- CRUD, filter
- Transaction + row lock có điều kiện (đúng alias)
- Link / unlink Attendance khi cần
- KHÔNG chứa rule nghiệp vụ (quyền, validate status...) — service quyết định.
"""
...
# ===== ép mọi thao tác dùng DB alias này =====
ERP_DB_ALIAS = "erp_postgres"


# ============================
# Helpers (FOR UPDATE có điều kiện)
# ============================
def _maybe_for_update(qs: QuerySet) -> QuerySet:
    """Chỉ thêm SELECT ... FOR UPDATE nếu connection (erp_postgres) đang ở trong atomic block."""
    try:
...
# ============================
# Base queries (đọc trên erp_postgres)
# ============================
def base_qs() -> QuerySet[LeaveRequest]:
    return LeaveRequest.objects.using(ERP_DB_ALIAS).all()

def get_by_id(leave_id: int) -> LeaveRequest:
    return LeaveRequest.objects.using(ERP_DB_ALIAS).get(id=leave_id)

def get_or_none(leave_id: int) -> Optional[LeaveRequest]:
    return LeaveRequest.objects.using(ERP_DB_ALIAS).filter(id=leave_id).first()

def list_my(employee_id: int, statuses: Optional[List[int]] = None) -> QuerySet[LeaveRequest]:
    qs = LeaveRequest.objects.using(ERP_DB_ALIAS).filter(employee_id=employee_id)
    if statuses:
        qs = qs.filter(status__in=statuses)
    return qs.order_by("-created_at")

def list_pending_for_manager(date_from: Optional[str] = None, date_to: Optional[str] = None) -> QuerySet[LeaveRequest]:
    qs = LeaveRequest.objects.using(ERP_DB_ALIAS).filter(status=LeaveRequest.Status.SUBMITTED)
    if date_from:
...
def filter_leaves(filters: Dict[str, Any], order_by: Optional[List[str]] = None) -> QuerySet[LeaveRequest]:
    qs = LeaveRequest.objects.using(ERP_DB_ALIAS).all()
...
def save_fields(obj: LeaveRequest, patch: Dict[str, Any], allowed: Optional[Iterable[str]] = None) -> LeaveRequest:
    ...
    with transaction.atomic(using=ERP_DB_ALIAS):
        ...
        if fields:
            ...
            obj.save(using=ERP_DB_ALIAS, update_fields=fields)
        return obj


# ========== Attendance linking helpers ==========
def _unlink_attendances_for_leave(leave: LeaveRequest) -> None:
    # Bulk UPDATE: không dùng select_for_update (thừa & dễ dính outside-of-transaction)
    with transaction.atomic(using=ERP_DB_ALIAS):
        Attendance.objects.using(ERP_DB_ALIAS).filter(on_leave=leave).update(on_leave=None)
...
def _link_leave_to_attendance_on_approve(leave: LeaveRequest) -> None:
    ...
    """
    with transaction.atomic(using=ERP_DB_ALIAS):
        # 1) Gán on_leave bằng bulk update (không cần FOR UPDATE)
        base = Attendance.objects.using(ERP_DB_ALIAS).filter(
            employee_id=leave.employee_id,
            date__gte=leave.start_date,
            date__lte=leave.end_date,
        )
        base.update(on_leave=leave)

        # 2) Iterate bằng queryset mới, có lock nếu đang trong txn
        iter_qs = _maybe_for_update(
            Attendance.objects.using(ERP_DB_ALIAS).filter(
                employee_id=leave.employee_id,
                date__gte=leave.start_date,
                date__lte=leave.end_date,
                on_leave=leave,
...
def approve_and_link(leave_id: int, manager_id: int, do_link_attendance: bool) -> LeaveRequest:
    with transaction.atomic(using=ERP_DB_ALIAS):
        obj = _maybe_for_update(LeaveRequest.objects.using(ERP_DB_ALIAS)).get(id=leave_id)
        obj.status = LeaveRequest.Status.APPROVED
        obj.decision_ts = timezone.now()
        obj.decided_by = manager_id
        obj.save(using=ERP_DB_ALIAS, update_fields=["status", "decision_ts", "decided_by", "updated_at"])
